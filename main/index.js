...
console.log('✅ DOM cargado correctamente');}catch(error){console.error('Error crítico en DOMContentLoaded:',error);showErrorScreen('Error crítico al cargar la aplicación. Por favor, recarga la página.');}});function checkFirstVisit(){const firstVisit=localStorage.getItem('foxweb_first_visit');if(firstVisit==='false'){AppState.firstVisit=false;}}
...
function initApp(){console.log('🎯 Inicializando aplicación...');try{if(!AppState.dbData){showErrorScreen('No se pudo cargar la base de datos. Por favor, recarga la página.');return;}
checkForNewContent();hideLoading();determineInitialTabFromHash();renderAllTabs();initSearch();initModals();initNotificationCenter();initFloatingButtons();initSidebar();initCounters();AppState.isLoading=false;saveAppState();console.log('✅ Aplicación inicializada correctamente');}catch(error){console.error('Error crítico en initApp:',error);showErrorScreen('Error crítico al inicializar la aplicación. Por favor, recarga la página.');}}
function determineInitialTabFromHash(){try{const hash=window.location.hash.substring(1);if(hash){const validTabs=['Programas','Sistemas','Juegos','Extras','APKs'];if(validTabs.includes(hash)){AppState.currentTab=hash;AppState.previousHash=hash;return;}}
AppState.currentTab='Programas';window.location.hash='#Programas';AppState.previousHash='Programas';}catch(error){console.error('Error determinando pestaña inicial:',error);AppState.currentTab='Programas';window.location.hash='#Programas';AppState.previousHash='Programas';}}
function checkForNewContent(){if(!AppState.dbData)return;try{const currentHash=calculateDBHash(AppState.dbData);AppState.dbHash=currentHash;const savedHash=localStorage.getItem(CONFIG.dbVersionKey);if(!savedHash){localStorage.setItem(CONFIG.dbVersionKey,currentHash);return;}
if(currentHash!==savedHash){console.log('🆕 ¡Se detectaron cambios en la base de datos!');const oldData=getCachedDBData();const newData=AppState.dbData;const changes=calculateContentChanges(oldData,newData);if(changes.totalNew>0){createNewContentNotification(changes);if(changes.totalNew===1){showToast('¡Se ha agregado 1 nuevo contenido!','info');}else{showToast(`¡Se han agregado ${changes.totalNew} nuevos contenidos!`,'info');}}
localStorage.setItem(CONFIG.dbVersionKey,currentHash);cacheDBData(newData);}else{console.log('✅ La base de datos está actualizada');}}catch(error){console.error('Error verificando contenido nuevo:',error);}}
...
function calculateContentChanges(oldData,newData){const changes={programas:0,sistemas:0,juegos:0,extras:0,apks:0,totalNew:0};if(!oldData){return changes;}
...
function activateCurrentTab(){try{document.querySelectorAll('.tab-content').forEach(tab=>{tab.classList.remove('active');tab.setAttribute('aria-hidden','true');});const activeTab=document.getElementById(AppState.currentTab);if(activeTab){activeTab.classList.add('active');activeTab.setAttribute('aria-hidden','false');}
document.querySelectorAll('.tablink').forEach(btn=>{const tabName=btn.getAttribute('data-tab')||btn.textContent.trim()||btn.getAttribute('onclick')?.match(/'([^']+)'/)?.[1];const isActive=tabName===AppState.currentTab;btn.classList.toggle('active',isActive);btn.setAttribute('aria-selected',isActive);});updateHash(AppState.currentTab);}catch(error){console.error('Error activando pestaña:',error);}}
function updateHash(tabName){if(AppState.navigationLock)return;try{AppState.navigationLock=true;const newHash=`#${tabName}`;const currentHash=window.location.hash;if(currentHash!==newHash){AppState.previousHash=tabName;if(history.replaceState){history.replaceState(null,null,newHash);}else{window.location.hash=newHash;}}
setTimeout(()=>{AppState.navigationLock=false;},100);}catch(error){console.error('Error actualizando hash:',error);AppState.navigationLock=false;}}
function renderTab(tabId,items){const grid=document.getElementById(`grid-${tabId}`);if(!grid)return;try{grid.innerHTML='';if(!items||items.length===0){grid.innerHTML=`
            ...
            `;return;}
const fragment=document.createDocumentFragment();items.forEach((item,index)=>{const itemId=`${tabId.toLowerCase()}_${index}`;const card=createContentCard(item,tabId,itemId);if(card)fragment.appendChild(card);});grid.appendChild(fragment);initContentCardsEvents();}catch(error){console.error(`Error renderizando pestaña ${tabId}:`,error);grid.innerHTML=`
            <div class="empty-state" role="status">
        ...
        `;}}
function createContentCard(item,category,itemId){try{const template=document.getElementById('contentCardTemplate');if(template){return createCardFromTemplate(template,item,category,itemId);}
return createCardManually(item,category,itemId);}catch(error){console.error('Error creando card:',error);return null;}}
function createCardFromTemplate(template,item,category,itemId){try{const clone=template.content.cloneNode(true);const card=clone.querySelector('.content-card');if(!card)return createCardManually(item,category,itemId);card.dataset.id=itemId;card.dataset.category=category.toLowerCase();card.dataset.type=getItemType(item);const icon=card.querySelector('.card-icon i');if(icon){icon.className=item.icon;}
...
function createCardManually(item,category,itemId){try{const card=document.createElement('div');card.className='content-card';card.dataset.id=itemId;card.dataset.category=category.toLowerCase();card.dataset.type=getItemType(item);const showCopyLink=!(item.modal&&item.modal!=='null')&&item.enlace&&item.enlace!=='#';const mainBadge=item.badges&&item.badges.length>0?item.badges[0]:null;const remainingBadges=item.badges&&item.badges.length>1?item.badges.slice(1):[];card.innerHTML=`
        ...
        `;return card;}catch(error){console.error('Error creando card manualmente:',error);const errorCard=document.createElement('div');errorCard.className='content-card';errorCard.innerHTML=`
            <div class="card-content">
                <h3 class="card-title">Error cargando contenido</h3>
                <p class="card-description">No se pudo cargar este elemento.</p>
            </div>
        `;return errorCard;}}
function getDownloadButtonHTML(item,itemId){try{if(item.modal&&item.modal!=='null'){return `
                <button class="download-btn" onclick="openModal('${item.modal}')" aria-label="Descargar ${item.name}">
                    <i class="fa-solid fa-download" aria-hidden="true"></i>
                    Descargar
                </button>
            `;}else if(item.enlace&&item.enlace!=='#'){return `
                <a href="${item.enlace}" 
                   class="download-btn" 
                   target="_blank" 
            ...
            `;}}catch(error){console.error('Error generando botón de descarga:',error);return `
...
function clearSearch(){try{const searchInput=document.getElementById('mainSearch');const clearBtn=document.getElementById('clearSearch');if(searchInput){searchInput.value='';searchInput.focus();}
if(clearBtn){clearBtn.style.display='none';}
AppState.searchActive=false;updateSearchState();performSearch();}catch(error){console.error('Error limpiando búsqueda:',error);}}
function showNoResults(show){try{const currentGrid=document.getElementById(`grid-${AppState.currentTab}`);if(!currentGrid)return;let noResults=currentGrid.nextElementSibling;if(!noResults||!noResults.classList.contains('no-results')){noResults=document.createElement('div');noResults.className='no-results';noResults.setAttribute('role','status');noResults.innerHTML=`
                <i class="fa-solid fa-search" aria-hidden="true"></i>
                <h3>No se encontraron resultados</h3>
                <p>Intenta con otros términos de búsqueda</p>
            `;currentGrid.parentNode.insertBefore(noResults,currentGrid.nextSibling);}
noResults.style.display=show?'block':'none';}catch(error){console.error('Error mostrando mensaje de no resultados:',error);}}
function openTab(tabName){if(AppState.navigationLock||AppState.currentTab===tabName)return;try{AppState.currentTab=tabName;document.querySelectorAll('.tab-content').forEach(tab=>{tab.classList.remove('active');tab.setAttribute('aria-hidden','true');});const activeTab=document.getElementById(tabName);if(activeTab){activeTab.classList.add('active');activeTab.setAttribute('aria-hidden','false');}
...
saveAppState();}catch(error){console.error(`Error abriendo pestaña ${tabName}:`,error);}}
...
function openModal(modalId){try{const modal=document.getElementById(modalId);if(!modal){console.warn(`Modal ${modalId} no encontrado`);return;}
modal.style.display='flex';modal.setAttribute('aria-hidden','false');document.body.style.overflow='hidden';const focusable=modal.querySelector('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])');if(focusable)focusable.focus();if(modalId==='sugerenciaModal'){resetSuggestionForm();}}catch(error){console.error(`Error abriendo modal ${modalId}:`,error);}}
function closeModal(modalId){try{if(modalId){const modal=document.getElementById(modalId);if(modal){modal.style.display='none';modal.setAttribute('aria-hidden','true');}}else{document.querySelectorAll('.modal[style*="display: flex"]').forEach(modal=>{modal.style.display='none';modal.setAttribute('aria-hidden','true');});}
document.body.style.overflow='';}catch(error){console.error('Error cerrando modal:',error);}}
...
function handleSuggestionSubmit(e){e.preventDefault();try{const nombre=document.getElementById('nombreSugerencia').value.trim();const descripcion=document.getElementById('descripcionSugerencia').value.trim();const categoria=document.getElementById('categoriaSugerencia').value;const enlace=document.getElementById('enlaceSugerencia').value.trim();const web=document.getElementById('webSugerencia').value.trim();const email=document.getElementById('emailSugerencia').value.trim();if(!nombre||!descripcion||!categoria||!enlace){showToast('Por favor, completa todos los campos requeridos.','error');return;}
...
showSuggestionConfirmation();saveSuggestionLocal({nombre,descripcion,categoria,enlace,web,email,fecha:new Date().toISOString(),estado:'pendiente'});openGitHubIssue(nombre,descripcion,categoria,enlace,web,email);}catch(error){console.error('Error enviando sugerencia:',error);showToast('Error al enviar la sugerencia. Por favor, inténtalo de nuevo.','error');}}
...
function saveSuggestionLocal(suggestion){try{let suggestions=JSON.parse(localStorage.getItem('foxweb_sugerencias'))||[];suggestion.id=Date.now();suggestions.push(suggestion);if(suggestions.length>100){suggestions=suggestions.slice(-100);}
localStorage.setItem('foxweb_sugerencias',JSON.stringify(suggestions));}catch(error){console.error('Error guardando sugerencia:',error);}}
function openGitHubIssue(nombre,descripcion,categoria,enlace,web,email){try{const title=`[SUGERENCIA] ${nombre}`;const body=`
...
*Sugerencia enviada desde [FoxWeb](https://foxweb.pages.dev)*
        `;const encodedTitle=encodeURIComponent(title);const encodedBody=encodeURIComponent(body);const issueUrl=`https://github.com/foxorange224/foxorange224.github.io/issues/new?title=${encodedTitle}&body=${encodedBody}&labels=sugerencia`;setTimeout(()=>{window.open(issueUrl,'_blank');},1000);}catch(error){console.error('Error abriendo issue de GitHub:',error);showToast('Error al abrir GitHub. Por favor, inténtalo manualmente.','error');}}
...
function toggleFavorite(itemId){try{if(AppState.favorites.has(itemId)){AppState.favorites.delete(itemId);showToast('Removido de favoritos','info');}else{AppState.favorites.add(itemId);showToast('Agregado a favoritos','success');}
updateFavoriteIconForItem(itemId);saveFavorites();saveAppState();}catch(error){console.error('Error alternando favorito:',error);showToast('Error al actualizar favoritos','error');}}
...
function copyItemLink(itemId){try{const item=findItemById(itemId);if(!item){showToast('No se pudo encontrar el item','error');return;}
...
navigator.clipboard.writeText(urlToCopy).then(()=>showToast('Enlace copiado al portapapeles','success')).catch(err=>{console.error('Error copiando al portapapeles:',err);const textArea=document.createElement('textarea');textArea.value=urlToCopy;document.body.appendChild(textArea);textArea.select();try{document.execCommand('copy');showToast('Enlace copiado al portapapeles','success');}catch(e){showToast('Error copiando enlace','error');}
document.body.removeChild(textArea);});}catch(error){console.error('Error copiando enlace:',error);showToast('Error al copiar enlace','error');}}
...
function setTheme(theme){try{AppState.theme=theme;document.documentElement.setAttribute('data-theme',theme);localStorage.setItem('foxweb_theme',theme);const themeToggle=document.getElementById('themeToggle');if(themeToggle){const icons=themeToggle.querySelectorAll('i');icons[0].style.display=theme==='dark'?'none':'block';icons[1].style.display=theme==='dark'?'block':'none';}
saveAppState();}catch(error){console.error('Error estableciendo tema:',error);}}
...
function renderNotifications(){const container=document.querySelector('.notification-list');if(!container)return;try{container.innerHTML='';if(AppState.notifications.length===0){container.innerHTML=`
                <div class="notification-empty" role="status">
                    <i class="fa-solid fa-bell-slash" aria-hidden="true"></i>
                    <p>No hay notificaciones</p>
                </div>
            `;return;}
const sortedNotifications=[...AppState.notifications].sort((a,b)=>new Date(b.date)-new Date(a.date));sortedNotifications.forEach(notification=>{const notificationEl=createNotificationElement(notification);container.appendChild(notificationEl);});}catch(error){console.error('Error renderizando notificaciones:',error);container.innerHTML=`
            <div class="notification-empty" role="status">
                <i class="fa-solid fa-exclamation-triangle" aria-hidden="true"></i>
                <p>Error cargando notificaciones</p>
            </div>
        `;}}
function createNotificationElement(notification){try{const div=document.createElement('div');div.className=`notification-item ${notification.read?'':'new'}`;div.setAttribute('role','listitem');const icon=getNotificationIcon(notification.type);const time=formatNotificationTime(notification.date);div.innerHTML=`
        ...
        `;return div;}catch(error){console.error('Error creando elemento de notificación:',error);const div=document.createElement('div');div.className='notification-item';div.innerHTML='<p>Error cargando notificación</p>';return div;}}
...
function initEventListeners(){try{const scrollTopBtn=document.getElementById('scrollTopBtn');if(scrollTopBtn){scrollTopBtn.addEventListener('click',scrollToTop);window.addEventListener('scroll',()=>{const show=window.scrollY>500;scrollTopBtn.classList.toggle('show',show);});}
window.addEventListener('hashchange',handleHashChange);window.addEventListener('online',()=>{AppState.isOffline=false;showToast('Conexión restablecida','success');});window.addEventListener('offline',()=>{AppState.isOffline=true;showToast('Estás sin conexión','warning');});}catch(error){console.error('Error inicializando event listeners:',error);}}
...
function initAccessibility(){try{document.addEventListener('keydown',(e)=>{if((e.ctrlKey||e.metaKey)&&e.key==='k'){e.preventDefault();const searchInput=document.getElementById('mainSearch');if(searchInput){searchInput.focus();searchInput.select();}}
if((e.ctrlKey||e.metaKey)&&e.key==='n'){e.preventDefault();openModal('sugerenciaModal');}
if(e.ctrlKey||e.metaKey){const tabKeys={'1':'Programas','2':'Sistemas','3':'Juegos','4':'Extras','5':'APKs'};if(tabKeys[e.key]&&tabKeys[e.key]!==AppState.currentTab){e.preventDefault();openTab(tabKeys[e.key]);}}});document.querySelectorAll('[tabindex]').forEach(el=>{el.addEventListener('keydown',(e)=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();el.click();}});});}catch(error){console.error('Error inicializando accesibilidad:',error);}}
...
function showToast(message,type='info'){try{const toast=document.createElement('div');toast.className=`toast toast-${type}`;toast.setAttribute('role','alert');toast.setAttribute('aria-live','assertive');const icon=getToastIcon(type);toast.innerHTML=`
            <i class="${icon}" aria-hidden="true"></i>
            <span>${message}</span>
            <button onclick="this.parentElement.remove()" aria-label="Cerrar notificación">
                <i class="fa-solid fa-times" aria-hidden="true"></i>
            </button>
        `;document.body.appendChild(toast);setTimeout(()=>{if(toast.parentNode){toast.parentNode.removeChild(toast);}},3000);}catch(error){console.error('Error mostrando toast:',error);}}
...
function showErrorScreen(message){try{hideLoading();document.querySelector('.page-container')?.style.setProperty('display','none','important');const errorScreen=document.getElementById('errorScreen');const errorMessage=document.getElementById('errorMessage');if(errorScreen&&errorMessage){errorMessage.textContent=message;errorScreen.classList.add('show');errorScreen.setAttribute('aria-hidden','false');const retryBtn=document.getElementById('retryBtn');const reportBtn=document.getElementById('reportBtn');if(retryBtn){retryBtn.onclick=function(){location.reload();};}
if(reportBtn){reportBtn.onclick=function(){closeErrorScreen();openModal('sugerenciaModal');};}}}catch(error){console.error('Error mostrando pantalla de error:',error);document.body.innerHTML=`
        ...
        `;}}
function closeErrorScreen(){try{const errorScreen=document.getElementById('errorScreen');if(errorScreen){errorScreen.classList.remove('show');errorScreen.setAttribute('aria-hidden','true');}
document.querySelector('.page-container')?.style.removeProperty('display');}catch(error){console.error('Error cerrando pantalla de error:',error);}}
